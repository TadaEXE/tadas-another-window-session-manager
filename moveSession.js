...
var MoveSession = class {
  ...
  constructor() {
    ...
    this._move_strategy = "strict";
    this._supported_move_strategies = Object.freeze(['strict', 'weak', 'app_name']);

  }

  async moveWindows(sessionName, move_strategy = "strict") {
    if (!sessionName) {
      sessionName = this.sessionName;
    }

    if (!this._supported_move_strategies.includes(move_strategy)) {
      logError(new Error(`Unsupported move strategy ${move_strategy} was chosen`));
      return;
    }
    this._move_strategy = move_strategy;
  ...
  _restoreWindowGeometry(metaWindow, saved_window_session) {
    ...
    if (delay) {
      // Fix: https://github.com/nlpsuge/gnome-shell-extension-tadas-another-window-session-manager/issues/25
  ...
  _do_move_strategy_weak(window, session) {
    let window_title = window.get_title()
    let session_title = session.window_title;
    if (session_title.startsWith("\\*") || session_title.endsWith("*\\")) {
      let clean_title = session_title.replaceAll(/\\/g, "");
      return window_title.includes(clean_title);
    }
    // If there is no weak match we can still try to get a strict one
    return this._do_move_strategy_strict(window, session);
  }

  _do_move_strategy_strict(window, session) {
    return window.get_title() === session.window_title;
  }

  _do_move_strategy_app_name(window, session) {
    return window.get_wm_class() === session.wm_class;
  }

  _get_move_strategy_result(window, session) {
    switch (this._move_strategy) {
      case this._supported_move_strategies[0]: // strict
        return this._do_move_strategy_strict(window, session);
      case this._supported_move_strategies[1]: // weak
        return this._do_move_strategy_weak(window, session);
      case this._supported_move_strategies[2]: // app_name
        return this._do_move_strategy_app_name(window, session);
      default:
        // At this point we shouldn't even be able to reach this so we just go with strict as it is the default
        return this._do_move_strategy_strict(window, session);
    }
  }
  ...
  _getAutoMoveInterestingWindows(shellApp, saved_window_sessions) {
    ...
    saved_window_sessions.forEach(saved_window_session => {
      open_windows.forEach(open_window => {
        ...
        if (windows_count === 1 || this._get_move_strategy_result(open_window, saved_window_session)) {
          if (open_window_workspace_index === desktop_number) {
