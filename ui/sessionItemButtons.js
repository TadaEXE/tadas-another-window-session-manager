...
var SessionItemButtons = GObject.registerClass(
  class SessionItemButtons extends GObject.Object {
    ...
    addButtons() {
      ...
      const moveStrictButton = this._addButton('move-symbolic.svg');
      moveStrictButton.set_reactive(this.sessionItem._available);
      new Tooltip.Tooltip({
        parent: moveStrictButton,
        markup: 'Move windows to their workspace by the saved session',
      });
      moveStrictButton.connect('clicked', this._onClickMove.bind(this, 'strict'));

      const moveWeakButton = this._addButton('seperator-symbolic.svg');
      moveStrictButton.set_reactive(this.sessionItem._available);
      new Tooltip.Tooltip({
        parent: moveWeakButton,
        markup: 'Move windows to their workspace by the saved session',
      });
      moveWeakButton.connect('clicked', this._onClickMove.bind(this, 'weak'));

      const moveAppNameButton = this._addButton('choose-window-symbolic.svg');
      moveStrictButton.set_reactive(this.sessionItem._available);
      new Tooltip.Tooltip({
        parent: moveAppNameButton,
        markup: 'Move windows to their workspace by the saved session',
      });
      moveAppNameButton.connect('clicked', this._onClickMove.bind(this, 'app_name'));
    ...
    _onClickMove(strategy) {
      this._moveSession.moveWindows(this.sessionItem._filename, strategy);
    }
